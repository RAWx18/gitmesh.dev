...
To deploy to staging, merge to the `staging-main` branch and push the code. The deployment will go through automatically! You can see the [GitHub staging deployment actions to check](https://github.com/GitMesh CE/GitMesh CE/actions/workflows/staging-deploy-backend.yaml) the status. You will also get a notification in the `deploys-staging` Slack channel once it has gone through.
...
Once the task has been finalized, it's time to merge and deploy! Deploying to production is simple: it's done with the [production deploy GitHub action](https://github.com/GitMesh CE/GitMesh CE/actions/workflows/production-deploy.yaml).&#x20;
